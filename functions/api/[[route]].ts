...
import { createJwt, extractBearerToken, verifyJwt } from "./auth";
...
const authMiddleware = async (c: any, next: any) => {
  const raw = c.req.header("Authorization") || null;
...
app.post("/subscribe", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (results && results.length > 0) {
      ...
      await c.env.DB.prepare(
        "UPDATE push_subscriptions SET username = ?, keys_p256dh = ?, keys_auth = ? WHERE endpoint = ?",
      )
      ...
      await c.env.DB.prepare(
        "INSERT INTO push_subscriptions (username, endpoint, keys_p256dh, keys_auth) VALUES (?, ?, ?, ?)",
      )
...
app.post("/ping", authMiddleware, async (c) => {
  ...
  try {
    await c.env.DB.prepare(
      "UPDATE push_subscriptions SET last_active_date = ? WHERE username = ?",
    )
      .bind(today, username)
      .run();
...
app.get("/daily-words", async (c) => {
  const today = new Date().toISOString().split("T")[0];
  try {
    const { results: dailyRec } = await c.env.DB.prepare(
      "SELECT word_ids FROM daily_words ORDER BY date DESC LIMIT 1",
    ).all();
