  ...
  async fetch(request: any, env: Env, ctx: any) {
    ...
    if (url.pathname === '/force-update-push') {
      ...
      try {
        ...
        if (subscriptions && subscriptions.length > 0 && env.VAPID_PRIVATE_KEY) {
          ...
          for (const sub of subscriptions) {
            try {
              const { endpoint, headers, body } = await buildPushHTTPRequest({
                privateJWK,
                subscription: { endpoint: sub.endpoint, keys: { p256dh: sub.keys_p256dh, auth: sub.keys_auth } },
                message: {
                  payload: {
                    title: `应用更新通知`,
        ...
        return new Response(`Update push for ${version} triggered manually.`, { status: 200 });
      } catch (err: any) {
        return new Response("Update push failed: " + err.message, { status: 500 });
      }
    }
